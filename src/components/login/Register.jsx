...
const FORM_CLIENTE_INICIAL = { nombre: '', email: '', telefono: '', password: '', confirm: '', consentimiento: false };
const FORM_EMPRESA_INICIAL = { empresa: '', rubro: RUBROS[0].tipoServicio, rut: '', direccion: '', comuna: '', telefono: '', email: '', password: '', confirm: '', terminos: false };
...
const Register = () => {
  ...
  const handleCliente = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('user', JSON.stringify({ id: usuarioCreado.id, email: usuarioCreado.correo, name: usuarioCreado.nombre, role: 'cliente' }));
  ...
  const switchTo = (t) => { setErrorCliente(''); setErrorEmpresa(''); setTipo(t); };
              ...
              <Form onSubmit={handleCliente}>
                ...
                <Form.Group className="mb-4">
                  <Form.Check
                    ...
                    label={<span>He leído y acepto la <Link to="/politica-privacidad" target="_blank" rel="noopener noreferrer">Política de Privacidad</Link> y autorizo el tratamiento de mis datos personales conforme a la Ley N° 19.628</span>}
                    checked={cliente.consentimiento}
                    onChange={e => campoCliente('consentimiento', e.target.checked)}
                  />
                </Form.Group>
                <button type="submit" className="reg-btn reg-btn--cliente" disabled={loadingCliente}>
              ...
              <Form onSubmit={handleEmpresa}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Select value={empresa.rubro} onChange={e => campoEmpresa('rubro', e.target.value)}>
                    {RUBROS.map(r => <option key={r.tipoServicio} value={r.tipoServicio}>{r.label}</option>)}
                  </Form.Select>
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label>RUT empresa</Form.Label>
                  <Form.Control placeholder="12345678-9" value={empresa.rut} onChange={e => campoEmpresa('rut', e.target.value)} required />
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label>Dirección</Form.Label>
                  <Form.Control placeholder="Av. Ejemplo 1234" value={empresa.direccion} onChange={e => campoEmpresa('direccion', e.target.value)} required />
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label>Comuna</Form.Label>
                  <Form.Select value={empresa.comuna} onChange={e => campoEmpresa('comuna', e.target.value)}>
                ...
                <Form.Group className="mb-4">
                  <Form.Check
                    type="checkbox"
                    id="terminos"
                    label={<span>Acepto los términos y condiciones de uso y la <Link to="/politica-privacidad" target="_blank" rel="noopener noreferrer">Política de Privacidad</Link> sobre el tratamiento de mis datos personales (Ley N° 19.628)</span>}
                    checked={empresa.terminos}
                    onChange={e => campoEmpresa('terminos', e.target.checked)}
                  />
                </Form.Group>
                <button type="submit" className="reg-btn reg-btn--empresa" disabled={loadingEmpresa}>
